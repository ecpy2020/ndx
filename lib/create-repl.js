...
function createRepl(inspector) {
    ...
    class CallFrame {
        ...
        loadScopes() {
            return Promise.all(
                this.scopeChain.filter((scope) => scope.type !== 'global').map((scope) => {
    ...
    function setBreakpoint(script, line, condition, silent) {
        ...
        return Debugger.setBreakpointByUrl({ urlRegex, lineNumber: line - 1, condition }).then((bp) => {
    ...
    function restoreBreakpoints() {
        ...
        const newBreakpoints = lastBreakpoints
            ...
            .map(({ location }) => setBreakpoint(location.scriptUrl, location.lineNumber + 1));
